...
Every graphic is generated at runtime: each card bears a constellation-sigil drawn deterministically from its name, an aurora breathes behind the city, stars twinkle among the drifting embers, a vast working turns slowly beneath the whole table — and along the bottom of the page, the gaslit city itself: two ridges of procedurally-built rooftops, spires, flickering windows, low fog, and a patient moon. Typography uses only fonts already on your system (engraved caps for headings, a bookish serif for prose, a clear humanist face for small UI text). Each new story opens with a short prologue at the graveside.
...
1. **Idle, not urgent** — actions collect themselves, loops repeat themselves, and the Chronicle remembers so you never have to react. Time stops only for dark turns.
